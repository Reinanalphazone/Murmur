@echo off
REM Development script for Murmur - sets up MSVC environment
REM Uses release mode to avoid UCRT debug assertion errors from native libs
...
echo.
echo Environment ready! Starting Tauri dev server (release mode)...
echo NOTE: Using release mode to avoid UCRT debug assertions from native libs.
echo.

cd /d d:\Repos\Whisper\murmur
